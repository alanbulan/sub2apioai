...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("loads a masked Codex proxy-pool node and submits its replacement URL", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      openai_codex_ticket_proxy_pool: [{
        id: "primary",
        name: "Primary",
        url: "http://user:***@old.example.com:8080",
        enabled: true,
        priority: 0,
        weight: 100,
    ...
    const input = wrapper.get<HTMLInputElement>(
      'input[aria-label="admin.settings.gatewayForwarding.codexTicketProxyURL"]',
    );
    ...
    await wrapper.find("form").trigger("submit.prevent");
    await flushPromises();
    expect(updateSettings.mock.calls[0]?.[0].openai_codex_ticket_proxy_pool).toEqual([{
      id: "primary",
      name: "Primary",
      url: "socks5h://user:new-secret@new.example.com:1080",
      enabled: true,
      priority: 0,
      weight: 100,
      rotate_on_failure: true,
